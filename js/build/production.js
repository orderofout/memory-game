var d,
	Database = {
		...
		init: function() {
			...
			Database.exec("CREATE TABLE IF NOT EXISTS userResults (id INTEGER PRIMARY KEY AUTOINCREMENT, firstName TEXT, lastName TEXT, completionTime INTEGER, score INTEGER, attempts INTEGER)", []);
	...
	Leaderboard = {
		...
		createTable: function() {
			...
			for(var x=0;x<lb.roster.length;x++){
				...
				var html = "<tr>";
		  			html += "<td>"+(x+1)+"</td>";
			  		html += "<td>"+lb.roster[x].firstName+ " "+ lb.roster[x].lastName+"</td>";
			  		html += "<td>"+lb.roster[x].score+"pts</td>";
			  		html += "<td>"+lb.roster[x].completionTime+"s</td>";
			  		html += "<td>"+lb.roster[x].attempts+"</td>";
			  		html += "</tr>";				
